...
class EmployeesController extends Controller
{
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Employees", "create")) {
			...
			\Session::flash('success_message', 'Panitia telah ditambahkan');
			...
			if(env('MAIL_USERNAME') != null && env('MAIL_USERNAME') != "null" && env('MAIL_USERNAME') != "") {
				...
				Mail::send('emails.send_login_cred', ['user' => $user, 'password' => $password], function ($m) use ($user) {
					...
					$m->to($user->email, $user->name)->subject('Informasi Akun Panitia Olimpiade TI 2017');
				});
			} else {
	...
	public function show($id)
	{
		if(Module::hasAccess("Employees", "view")) {
			...
			if((isset($employee->id) && Auth::user()->context_id == $id) || Auth::user()->type == "Administrator") {
				$module = Module::get('Employees');
				$module->row = $employee;
